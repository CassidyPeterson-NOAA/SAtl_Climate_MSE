---
title: "SAtl_Climate_MSE_Results - Black Sea Bass Overages"
...
Below are the results for BSB with implementation overages (BSB_O). 
...
```{r BSB_O-indices-plots, echo=FALSE, fig.height=5, fig.cap = "Observed indices of abundance"}
base<-get(sp)[["base"]]
...
yrs<-uobsHi@PPD$SCA_1@Year
ey<-length(yrs)
obj<-uobsHi@PPD$SCA_1@AddInd[1,1,]
by<-tail(which(is.na(obj)),1)+1
for(i in 100:111){
  plot(yrs[by:ey], uobsHi@PPD$SCA_1@AddInd[i,1,by:ey], type='l', col='red', ylab="Index",xlab="Year")
  lines(yrs[by:ey], uobsLo@PPD$SCA_1@AddInd[i,1,by:ey], type='l', col='deepskyblue')
  lines(yrs[by:ey], base@PPD$SCA_1@AddInd[i,1,by:ey], type='l')
}
legend("topright", c("base","uobsHi","uobsLo"), lty=1, lwd=1, col=c('black','red','deepskyblue'), bty='n')
par(mfrow=c(1,2))
