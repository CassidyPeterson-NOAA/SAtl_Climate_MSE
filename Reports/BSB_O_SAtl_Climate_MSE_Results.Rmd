---
title: "Climate-readiness of empirical vs. model-based MPs in the US South Atlantic - Black Sea Bass with Overages"
...
This section is focused on results for black sea bass *with implementation overages*. 
...
* Case study species
  + VS - vermilion snapper, 'VS_O' indicates vermilion snapper with implementation overages
  + BSB - black sea bass, 'BSB_O' indicates black sea bass with implementation overages
  + RP- red porgy, 'RP_O' indicates red porgy with implementation overages
...
# Black Sea Bass with Implementation Overages Results

```{r BSB_O-FOUNDATION, setup, include=FALSE, cache=TRUE, cache.lazy=FALSE}
...
```{r BSB_O-biomass1-PM, echo=FALSE, fig.height=5, fig.width=9, fig.cap = "Relative $SSB/SSB_{MSY}$ at year 30 by MP for each OM."}
...
```{r BSB_O-F1-PM, echo=FALSE, fig.height=5, fig.width=9, fig.cap = "Relative $F/F{MSY}$ at year 30 by MP for each OM."}
par(mfrow=c(3,3), mar=c(2.4, 2.4, 0.2, 0.2), mgp=c(1.1, 0.25, 0), tck=-0.01, cex.axis=1)
myPlot_Violin(get(paste0(sp,"_PM_nest")), stat='relF30', ylimsEpiM=c(0,25), legend=F, mf=c(3,3))
```


```{r BSB_O-F2-PM, echo=FALSE, fig.height=5, fig.width=9, fig.cap = "Probability of not overfishing (PNOF) across the 50-year projection period by MP for each OM."}
par(mfrow=c(3,3), mar=c(2.4, 2.4, 0.2, 0.2), mgp=c(1.1, 0.25, 0), tck=-0.01, cex.axis=1)
...
```{r BSB_O-Yield1-PM, echo=FALSE, fig.height=5, fig.width=9, fig.cap = "Relative yield (yield / MSY) at projection year 30 by MP for each OM."}
 par(mfrow=c(3,3), mar=c(2.4, 2.4, 0.2, 0.2), mgp=c(1.1, 0.25, 0), tck=-0.01, cex.axis=1)
 myPlot_Violin(get(paste0(sp,"_PM_nest")), stat='yield30', legend=F, mf=c(3,3), ylimsrecns=c(0,15))
```


```{r BSB_O-Yield2-PM, echo=FALSE, fig.height=5, fig.width=9, fig.cap = "Cumulative relative yield (yield / MSY) at projection year 30 by MP for each OM."}
 par(mfrow=c(3,3), mar=c(2.4, 2.4, 0.2, 0.2), mgp=c(1.1, 0.25, 0), tck=-0.01, cex.axis=1)
 myPlot_Violin(get(paste0(sp,"_PM_nest")), stat='cyield30', refline1=30, legend=F, mf=c(3,3), ylimsrecns=c(0,500))
```
...
```{r BSB_O-cumulative-PM-plot, echo=FALSE, fig.height=5, fig.width=7, fig.cap = "MP performance across OMs"}
