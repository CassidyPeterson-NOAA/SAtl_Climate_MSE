...
MPs_user_RP <- c("SCA_1", "pMP_5", "pMP_10",
                 ...
                 "myICI_RP", "myICI2_RP", "myIratio_RP",
...
## test slope runs
MPs_user_BSB3 <- c("GB_slope_BSB", "myIslope_BSB")
MPs_user_RP3 <- c("GB_slope_RP", "myIslope_RP")
MPs_user_VS3 <- c("GB_slope_VS", "myIslope_VS")
MPs_user_interval3<-c(1, 1)
...
OMName<-c("OM_RedPorgy")
OMName_O<-c("OM_RedPorgy_Over")
...
for(OMName_k in OMNames)       { ######### Loop over operating model
  ...
  if(OMName_k=="OM_BlackSeaBass" | OMName_k=="OM_BlackSeaBass_Over"){
    MPs_user_k <- MPs_user_BSB
    MPs_user_k3 <- MPs_user_BSB3
  }
  if(OMName_k=="OM_RedPorgy" | OMName_k=="OM_RedPorgy_Over"){
    MPs_user_k <- MPs_user_RP
    MPs_user_k3 <- MPs_user_RP3
  }
  if(OMName_k=="OM_VermilionSnapper" | OMName_k=="OM_VermilionSnapper_Over"){
    MPs_user_k <- MPs_user_VS
    MPs_user_k3 <- MPs_user_VS3
  }
  ...
  for(scenario_i in scenario) { ######### Loop over scenario
    ...
    # if(!OMName_scen%in%OMName_scen_complete){
      ...
      set.seed(myseed)
      sfExport(list = c("Assess_diagnostic_NK","SCA_NK","MSY_frac",MPs_user_k, MPs_user_k3))
      ...
      #                       MPs = MPs_user2,
      #                       parallel = runMSE_args$parallel, extended=runMSE_args$extended, silent=runMSE_args$silent)
      OM_k@interval <- MPs_user_interval3
      set.seed(myseed)
      MSE_batch_3 <- runMSE(OM_k,
                            MPs = MPs_user_k3,
                            parallel = runMSE_args$parallel, extended=runMSE_args$extended, silent=runMSE_args$silent)


      # MSE_batch<-merge_MSE(MSE_batch_1, MSE_batch_2)
      MSE_batch<-MSE_batch_3
      ...
      saveRDS(res,file = paste0("MSE_obj/", MSEName_k, "_", scenario_i, "SLOPE3.rds"))
