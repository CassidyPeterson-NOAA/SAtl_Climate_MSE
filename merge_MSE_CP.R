...
merge_MSE <- function(...) {
  ...
  maxage <- dots[[1]]@Hist@OM@maxage ## CP Added
  nareas <- length(dots[[1]]@Hist@OM@Size_area_1) ## CP Added
  ...
  slotvec <- c("SB_SBMSY", "F_FMSY", "B", "SSB", "VB", "FM", "Catch", "Removals", "Effort",
               "TAC", "TAE")
  res <- list()
  ...
  # For N (nsim, maxage+1, nMPs, proyears, nareas) ########## CP ADDED

  resN <- list()
  new_matN <- array(NA, dim = c(nsim, maxage+1, sum(nMPs), proyears, nareas)) ## CP added
  for(j in 1:length(dots)) {
    array_ij <- getElement(dots[[j]], "N")
    if(j == 1) new_matN[, , 1:nMPs[1], , ] <- array_ij
    if(j > 1) new_matN[, , (sum(nMPs[1:(j-1)]) + 1):(sum(nMPs[1:(j-1)]) + nMPs[j]), , ] <- array_ij
  }
  resN <- new_matN
